...
- **レーン自動移動(レイヤーモデル — ゴール文書「レーン自動移動」節に従う)**:
  - **L2 プロセス検知(全カード共通)**: tmux のペイン前面プロセス
    ...
    条件には L2 のプロセス開始も含める)。
  - **L3 Claude hook(L2 に優先)**: **PATH シム方式** — カードのシェルの
    PATH 先頭にシムディレクトリを置き、`claude` 実行時に `--session-id` の
    生成と hook 設定(`--settings`)の注入を自動で行う。カード ID はサーバーが
    シェルへ渡す環境変数から取得し、hook の POST に含めてカードへ紐づける。
    **シム方式が成立しない事実が判明した場合は作業を止めて利用者に確認する**
    (代替: 元祖方式のグローバル hook マージ)。
- **既存セッションの取り込み**: プロジェクト登録時に `/api/sessions?cwd=` の
  resume 候補を一覧し、選んだものをカード化。

**論点2 の作業既定値**(`docs/09-open-questions.md` 論点2・論点7): 上記の
PATH シム方式。**利用者がカードのシェル内で手打ちした `claude` にも hook が
効き、カードへ自動紐づけされること**が受け入れ条件。
