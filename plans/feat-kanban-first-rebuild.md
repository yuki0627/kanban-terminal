...
**仕様の空白は、遅延起動・省リソース・元仕様の踏襲といった工学的デフォルトで
埋めない。** `docs/00-goal.md` の「価値基準」節(UX 4 原則)から導出するか、
導出できなければ利用者に確認する。
...
- **カード作成**: プロジェクト(または Project なし)に対して作成。
  **種別選択は無い — カードのターミナルはただのシェル**(ゴール文書の
  レイヤーモデル L1。`$SHELL` を起動する。Claude を直接 spawn する経路は
  カードでは使わない)。
  **カード作成と同時にシェルを起動する — 「未起動のカード」という状態は無い**
  (2026-07-06 利用者決定・論点8)。作成した瞬間に tmux 上でシェルが立ち上がり、
  Archive まで生き続ける。カードを開く = 既に生きているターミナルへの接続
  (即座・ひと手間なし)。サーバー起動時やカード表示時にターミナルが死んで
  いたら(マシン再起動後など)自動でシェルを起動し直して不変条件を保つ。
  cwd はプロジェクトの root(Project なしはホームディレクトリ)。
- **カード名**: 手動が基本。エージェントセッションが紐づいたら会話タイトルを
  未命名カードの自動初期値に。メモ編集可。
- **レーン自動移動(レイヤーモデル — ゴール文書「レーン自動移動」節に従う)**:
  - **L2 プロセス検知(全カード共通)**: tmux のペイン前面プロセス
    (`#{pane_current_command}` 等)をサーバーが監視し、シェル以外が前面に
    来たら In Progress、シェルに戻ったら In Review。エッジトリガーで
    既存の `kanbanBoard.ts` 遷移ルールに接続する(手動完了保護の再オープン
    条件には L2 のプロセス開始も含める)。
  - **L3 PTY 活動検知(L2 に優先、フックなし)**: エージェント(claude /
    codex)が前面のときだけ、サーバーが既に中継している PTY の入力・出力を
    見て判定する。ユーザーの Enter 入力 → 1.5 秒以内に持続出力(3 チャンク
    以上)が始まったら working(In Progress)。working 中に出力が 2.0 秒
    より長く止まったら In Review(完了・承認待ちを区別しない)。Enter が
    先行しない出力(起動バースト・resume 再描画・アイドル中の孤立再描画)
    は working にしない。エージェント検出は `#{pane_current_command}` の
    単純一致では不十分(npm 版 claude は `node` になり得る)なので、ペイン
    PID からプロセスツリーを辿ってコマンドラインをマッチさせる。
    詳細な判定仕様・実測根拠は `docs/09-open-questions.md` 論点2、実装手順は
    `plans/feat-hookless-lane-signals.md` を参照。
- **既存セッションの取り込み**: プロジェクト登録時に `/api/sessions?cwd=` の
  resume 候補を一覧し、選んだものをカード化。session ID の紐づけは transcript
  発見方式(cwd スラグ配下の新規 jsonl を起動時刻の相関で発見)。

**論点2 の決定**(`docs/09-open-questions.md` 論点2・論点7): 上記の PTY 活動
検知方式(フックレス)。**利用者がカードのシェル内で手打ちした `claude` /
`codex` にも判定が効き、カードへ自動でレーン移動が反映されること**が
受け入れ条件。
...
- Archive UI: 案は選定済み(2026-07-07 決定)— **「レーン末尾の折りたたみ帯」**
  (`docs/ui-proposals/archive-ui-proposals.html` 案3)を実装する。
  5 レーンの右端に Archive の帯を常設(既定は折りたたみ: 薄い縦帯 + 件数)。
  展開するとグレースケール調の archived カードが並び、復元は帯から
  レーンへのドラッグで盤面上で完結する。検索/フィルタは最初は作らない
  (件数が増えて困ってから)。Archive への投入(カード上の操作・矩形選択の
  一括 Archive)はゴール文書どおりで、実行中プロセスがあるカードは
  確認ダイアログを出す(論点1)。
- 展開ウィンドウのサイズ・位置保存(利用者がドラッグで決めた値のみ保持 — 元祖仕様)
- 複数選択(ボード空白からの矩形選択)→ 一括 Archive
- unread マーカー(既存実装を維持)

**PR #3(フックレス差し替え)からの繰り越し** — レビュー(2026-07-07)で
非ブロッカーと判定し、マージを優先してこの Phase に繰り越した 3 点:

1. **セッション紐づけ + カード自動命名(未実装)**: transcript 発見方式
   (cwd スラグ配下の新規 jsonl を起動時刻の相関で発見)で claude セッションを
   カードへ紐づけ、jsonl の `ai-title` 行を未命名カードの自動初期値にする。
   仕様は `plans/feat-hookless-lane-signals.md`「セッション紐づけ」節。
   L4 resume(マシン再起動後の `claude --resume`)の受け皿もここで作る。
2. **軽微修正**: working 中(静止 2 秒以内)にエージェントを終了して前面が
   シェルに戻ると、done が発行されず In Progress に残るエッジがある。
   ポーリングのシェル復帰分岐で、エージェント状態が working のままなら
   done を発行してからクリアする。
3. **軽微修正**: エージェント検出の `AGENT_ARGS_RE` が `grep claude 〜` の
   ような「引数に claude / codex を単語として含む短命コマンド」を誤検知し得る。
   実害は小さいが、マッチ精度の改善は実装裁量で行ってよい。
...
- **AgentKind の継ぎ目**(論点4 決定): ①プロセス検出パターン(コマンドライン
  マッチ条件)②resume コマンド生成 ③カードタイトルの自動取得元、の 3 点だけの
  薄いインターフェースを切る(PTY 活動監視自体はエージェント種別を問わず共通)。
  実装は Claude Code のみ。Codex は作らない(別プランで対応)。
...
- `npx`(ローカルでは `yarn dev`)で起動すると看板ボードが開き、
  プロジェクト追加 → カード作成(と同時にシェルが起動)→ カードを開くと即接続 →
  (a) `sleep 5 && echo done` 等のコマンド実行で In Progress → 終了で
  In Review(L2)、(b) シェルで `claude` を手打ちして作業させると
  In Progress、入力待ちで In Review(L3 が L2 に優先)→ 手動で Done →
  Archive でターミナル解放(確認つき)、が一連で動く。
